...
while running:
    ...
    for bullet in arrows:
        index=0
        speed: int = 2
        if bullet_timer % update_interval == 0:
    ...
    time_left = 60000 - pygame.time.get_ticks()
    minutes = time_left // 60000
    seconds = (time_left // 1000) % 60
    survivedtext = font.render(f"{minutes}:{str(seconds).zfill(2)}", True, (0, 0, 0))
    textRect = survivedtext.get_rect()
    textRect.topright=[635,5]
    screen.blit(survivedtext, textRect)
    # 6.5 - Draw the Health bar
